...
#names the session login_session, which you will see throughout the code
#i think its the same as just calling it session, but it probably refers to
#something specific, ie the actual user's login session
...
@app.route('/login/')
def showLogin():
    #randomly creates a hash or string 32 characters long from either 0-9 
    #digits or uppercase ascii characters
    state = ''.join(random.choice(string.ascii_uppercase + string.
                digits) for x in xrange(32))
    #assigns the above state value to current login_session, used for
    #verification later
...
def fbconnect():
    ...
    #strip expire tag from access token since it's not need to make API calls
    data = json.loads(result)
    token = 'access_token=' + data['access_token']
    
    #if the token works, should be able to make API calls with new token like below:
    url = 'https://graph.facebook.com/v2.9/me?%s&fields=name,id,email' % token
    h = httplib2.Http()
    result = h.request(url, 'GET')[1]
    #populate login_session with user data from fb
    data = json.loads(result)
...
def deleteRestaurant(restaurant_id):
  ...
  if restaurantToDelete.user_id != login_session['user_id']:
      ...
      delete this.  Please login.');}</script><body onload=myFunction()''>"
  if request.method == 'POST':
